...
RSpec.feature 'OutpatientPrescriptions', type: :feature do
  before(:all) do
    ...
    PermissionUser.create(user: @farm_provider, sector: @farm_provider.active_sector,
                          permission: @create_patient_permission)
  end
  ...
  describe '', js: true do
    ...
    describe 'Form' do
      ...
      # it 'dispense on new record fails with duplicated products' do
      #   product = @products.sample
      #   2.times do |index|
      #     prod_quantity = rand(3..9)
      #     within '#order-product-cocoon-container' do
      ...
      #     sleep 1
      #     within '#order-product-cocoon-container' do
      #       page.execute_script %{$('input.request-quantity').last().val("#{prod_quantity}").keydown()}
      #       page.execute_script %{$('input.deliver-quantity').last().val("#{prod_quantity}").keydown()}
      ...
      #       page.execute_script %{$('input[name="lot-quantity[0]"]').last().click().val("#{prod_quantity}")}
      #       click_button 'Volver'
      #     end
      #     sleep 1
      #     click_link 'Agregar producto' unless (index + 1).eql?(2)
      #   end
      #   click_button 'Dispensar'
      #   expect(page).to have_text('Este producto ya se encuentra en la orden')
      # end
    ...
    # describe 'Form' do
    #   before(:each) do
    #     @op_pending = OutpatientPrescription.pendiente.sample
    #     visit "/recetas/ambulatorias/#{@op_pending.id}/editar"
    #   end

    #   it 'dispense record fails with duplicated products' do
    #     product = @op_pending.products.sample
