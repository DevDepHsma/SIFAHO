...
RSpec.feature 'InternalOrdersProviderFiltersSpec.rbs', type: :feature do
  ...
  describe '', js: true do
    ...
    describe 'Nullify permission' do
      before(:each) do
        internal_order = InternalOrder.by_provider(@farm_applicant.active_sector.id).solicitud.solicitud_enviada.where(provider_sector_id: @farm_applicant).sample
        within '#internal-filter' do
          fill_in 'filter[code]', with: internal_order.remit_code
          click_button 'Buscar'
          sleep 1
      ...
      #   click_link 'Anular'
      #   expect(page).to have_content('Solicitud se ha anulado correctamente.')
      # end
    end
  end
end
