Rails.application.routes.draw do
  localized do
    ...
    resources :permission_requests, except: [:edit, :update, :create, :destroy] do
      collection do
        get :request_sectors
    ...
    resources :users_admin, controller: :users, only: %i[index update show] do
      member do
        ...
        put :permissions_build_permission_from_role, to: 'permissions#build_permission_from_role', as: :build_permission_from_role
      end
    end
